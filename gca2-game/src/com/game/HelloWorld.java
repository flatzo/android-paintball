...
public class HelloWorld implements ApplicationListener, InputProcessor {
	...
	public void render() {
		...
		Vector3 worldToWindow = new Vector3(mRenderTree.getMainCharacter().getRelativePosition().x, mRenderTree.getMainCharacter().getRelativePosition().y, 0);
		
		//mCamera.project (worldToWindow);
		
	    //gl.glViewport(x, y, width, height);      
		gl.glViewport(
//				(int) (worldToWindow.x - Gdx.graphics.getWidth()/2 ),
//				(int) (worldToWindow.y - Gdx.graphics.getHeight()/2 ) ,
				(int) (-mRenderTree.getMainCharacter().getRelativePosition().x + Gdx.graphics.getWidth()/2 -128) ,//0,//(int) (Math.abs(worldToWindow.x)) ,//(int) (-worldToWindow.x + Gdx.graphics.getWidth()/2 ),
				(int) (-mRenderTree.getMainCharacter().getRelativePosition().y + Gdx.graphics.getHeight()/2 -128),//50,//(int) ( Gdx.graphics.getHeight() - worldToWindow.y ),
				(int) ( Gdx.graphics.getWidth()/2 + 256 ),//mRenderTree.getMainCharacter().getRelativePosition().x ),
				(int) ( Gdx.graphics.getHeight() ) 
				);
		//mCamera.translate (worldToWindow.x, worldToWindow.y,0);
//				glViewport.x + mRenderTree.getMainCharacter()
//				.getRelativePosition().x), (int) (glViewport.y + mRenderTree
//				.getMainCharacter().getRelativePosition().y),
//				(int) (glViewport.width + mRenderTree.getMainCharacter()
//						.getRelativePosition().x),
//				(int) (glViewport.height + mRenderTree.getMainCharacter()
//						.getRelativePosition().y));
		...
		/*
		 * spriteBatch.begin(); font.draw(spriteBatch, "FPS: " +
		 * Gdx.graphics.getFramesPerSecond(), 20, 20); font.draw(spriteBatch,
		 * "InitialCol, LastCol: " + mTileMapRenderer.getInitialCol() + "," +
		 * mTileMapRenderer.getLastCol(), 20, 40); font.draw(spriteBatch,
		 * "InitialRow, LastRow: " + mTileMapRenderer.getInitialRow() + "," +
		 * mTileMapRenderer.getLastRow(), 20, 60);
		 * 
		 * tmp.set(0, 0, 0);
		 * 
		 * //mCam.unproject(tmp); mCamera.unproject(tmp); font.draw(spriteBatch,
		 * "Location: " + tmp.x + "," + tmp.y, 20, 80); spriteBatch.end();
		 */
		...
		/*
		 * if (textPosition.x < 0) { textDirection.x = -textDirection.x;
		 * textPosition.x = 0; } if (textPosition.x > Gdx.graphics.getWidth()) {
		 * textDirection.x = -textDirection.x; textPosition.x =
		 * Gdx.graphics.getWidth(); } if (textPosition.y < 0) { textDirection.y
		 * = -textDirection.y; textPosition.y = 0; } if (textPosition.y >
		 * Gdx.graphics.getHeight()) { textDirection.y = -textDirection.y;
		 * textPosition.y = Gdx.graphics.getHeight(); }
		 * 
		 * spriteBatch.begin(); spriteBatch.setColor(Color.WHITE);
		 */
		/*
		 ...
		 * texture.getHeight());
		 * 
		 * spriteBatch.draw(texture, 10, 10); font.draw(spriteBatch, "hello",
		 * (int)textPosition.x, (int)textPosition.y); spriteBatch.end();
		 */
	...
	public boolean touchDown(int x, int y, int pointer, int button) {
		...
		if (mNumberOfFingers == 1) {
			...
			Vector2 distanceTravelled = mRenderTree.getMainCharacter()
					.moveToward(new Vector2(x, Gdx.graphics.getHeight() - y));

			//mCamera.focusOn(x, y /* mRenderTree.getMainCharacter() */, mTexture,
			//		spriteBatch);
